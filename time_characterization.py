...
#%% Funcions que fem servir

#funció per pasar dfs a arrays
def arrays(dfs,q_i,q_f,q_pas):
    dades = [df.to_numpy() for df in dfs]
    #valors de q que volem
    valors_q = np.arange(q_i,q_f,q_pas)
    n_q = len(valors_q)
    #numero de datasets
    n_n = len(dades)
    return dades, valors_q, n_q, n_n

#funcio per a calcular els moments
def moments(dades, valors_q, n_q, n_n ):
    #Càlcul de moments
    moms = np.zeros([n_q, n_n])
    for i in range (0,n_n):
        for j, q in enumerate(valors_q):
            moms[j,i] = np.mean(dades[i][:,6]**q)
    ln_moments= np.log(moms)
    return moms, ln_moments

#temps mitjà i error per a un grup x donat que tambe estan a y
...
#funcio que ens genera dos arrays, un de 1 a 5 i l'altre de 2 a 6
def taus_1(dades,grups):
    borrar = []
    for i in range (0, len(dades)):
        if (i%grups == 0):
            borrar.append(i)
    #pasarem a tenir dades agrupades per 5, ja que a x li traurem l'últim de cada joc perque no el podem comparar amb res    
    x = [val for i, val in enumerate(dades) if i not in borrar ]
    y = [val for i,val in enumerate(dades) if i+1 not in borrar]
    del y[-1]
    return np.array(x), np.array(y)

#generem dades seguint una distribució de Poisson amb mitjana igual a les dades experiementals.
def generador_poisson(dades,grups):
    tau_poiaux = []
    llargada = dades.shape[0]
    t_poiaux = np.zeros(llargada)
    for i in range (0,llargada):
        t_poiaux[i] = random.uniform(0,1)
        construir_tau = t_poiaux.copy()
        blocs = [construir_tau[i:i+grups] for i in range(0, len(construir_tau[:]), grups)]
        for array in blocs:
            array.sort()
    ...
    coef = np.average(tau_poi)/np.average(dades)
    tau_poi = tau_poi/coef
    return tau_poi

#generem dataset amb ordre canviat
def barrejar(dades):
...
#Mitjana interevent donat que l'anterior sigui més petit
def mitjana_condi(dades,grups,condicio):
    x,y = taus_1(dades,grups)
    llista = []
    mitjana = 0
    for i in range(0,len(x)):
        if y[i] < condicio and y[i] != 0:
            llista.append(x[i])
            mitjana = np.average(np.array(llista))
        # if count == 0:
        #     mitjana = 0
    return mitjana

#funció que cosntrueix densitats de probabilitat
...
#funció que calcula divergencia
...
#ho fem així ja que el primer de cada joc mai participa si tenim en compte la decissió d'abans
...
#condicio es la decisio anterior i index és la columna que volem comparar (genere, edat, etc)
#ens dona interevnt times i quants de cassos  tenim
...
#Condicio que triem donat grup d'edat
...
#calcul mitjana, variancia i incertesa mitjana donada una llista
def mitjana(llista):
    if len(llista) == 0:
        m = 0
    else:   
        m = sum(llista)/len(llista)
    return m

def variancia(llista):
    var = []
    for  i in range (0,len(llista)):
        var.append((llista[i]-mitjana(llista))**2)
    v = mitjana(var)  
    return v
...
def incert(llista):
    if len(llista) == 0:
        ...
        return np.sqrt(variancia(llista)/len(llista))
 
#Chi-squared test i Cramér's V, taula és uantaula de contingència
def cramers_v(taula):
...
#funcio que rtorna temps desde 0 per a cada joc
def temps(x,grups):
    blocs = [x[i:i+grups].copy() for i in range(0, len(x), grups)]
...
#construim funcio probabilitat decomulada
...
#funció que aplica covariancia generalitzada
...
#temps mitjà per a tots els grups
...
#temps mitjà grup x donat que y
...
#scatter poisson i experimentals junts
tau_poi = generador_poisson(dades[0][:,6],6)
...
#valor mitjana intervent donat el d'abans
valors = np.arange(0,np.max(dades[0][:,6]),0.1)
mitjanes = np.zeros(valors.shape[0])
for i, v in enumerate(valors):
      mitjanes[i] = mitjana_condi(dades[0][:,6],6,v)
       
plt.figure()
plt.plot(valors,mitjanes,'o',markersize=2)
plt.legend(frameon=False)
plt.tick_params(axis='both', which='both', top=True, right=True, direction='out')
plt.xlabel(r'$\tau_{i-1} \ (s)$')
...
plt.xlabel(r'$\tau_{i-1} \ (s)$')
plt.ylabel(r'$\langle \tau_i| \tau_{i-1} \rangle \ (s)$')
plt.savefig(f'mitjanes_poi_exp.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.xlabel(r'$\tau_{i-1} \ (s)$')
plt.ylabel(r'$\langle \tau_i| \tau_{i-1} \rangle \ (s)$')
plt.savefig(f'mitjanes_barr_poi.png', dpi=300, bbox_inches='tight')
plt.close()
...
densitat = estimador_densitat(dades_comulada,pas=0.5)
...
#         if ix2 > ix1:
#             p_fila.append(pvalue(ix1,ix2,N=10000))
#         else:
...
dades_t[:,6] = temps(dades[0][:,6],6)

t_b = temps(dades_barrejades[:,6],6)
t_poi = temps(tau_p,6)

llista = [np.sort(tau[:,6]),np.sort(tau_poi),np.sort(dades_t[:,6]),np.sort(t_poi)]
...
pcov = []
for i in range (0, len(etiquetes)):
    popt_i, pcov_i = curve_fit(ajust_multifractal, valors_q, ln_moms[:,i], maxfev=10000)
    popt.append(popt_i)
    pcov.append(pcov_i)
...
for i in range(0,len(etiquetes)):
    ...
    plt.plot(popt[i][1]*valors_q**popt[i][3],popt[i][1]/popt[i][2]*((1/valors_q)*ln_moms[:,i]-popt[i][0]),'o', markersize=1, color=colors[i],label=etiquetes[i])
# plt.xticks(np.arange(0,4,0.5))
# plt.yticks(np.arange(0,1.1,0.2))
# plt.xlim(0,4.0)
# plt.ylim(0,1.0)
